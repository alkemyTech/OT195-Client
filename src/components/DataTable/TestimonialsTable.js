...
const TestimonialsTable = () => {
  const [colDefs] = useState([ // con esto a poner cada nombre de la tabla y renderizar cada componente
    {
      title: "Nombre",
      field: "name",
    },
  ...
  // DataTable data
  const { data, loading , refetch } = useFetch( // traigo todos los testimonios
  ...
  const postForm = async (values) => { // metodo para el post de testimonio
    try{
      // console.log(values)
      const response = await fetch(`${process.env.REACT_APP_TESTIMONIALS_ENDPOINT}testimonials`,{
        method:"POST",
        headers:{
  ...
  const patchForm = (values) => {
    Swal.fire({
      ...
      if(result){
        try{
          const response = await fetch(`${process.env.REACT_APP_TESTIMONIALS_ENDPOINT}testimonials/${selectedRowData.id}`,
          {
            method: "PUT",
            headers:{
              Accept: "application/json",
        ...
        <Row>
          <Col>
            ...
            <FormModal name="Testimonio">   {/*es el nombre que va tener */}
              {showAdd ? <TestimonieForm fetchMethod={postForm}></TestimonieForm> : null}
              {showEdit ? (
                detailsLoading ? ( <Loader></Loader>) : 
                (<TestimonieForm values={detailsLoading ? [] : detailsData.result} fetchMethod={patchForm} ></TestimonieForm>) 
              ): null }
            </FormModal>
          </Col>
        </Row>
